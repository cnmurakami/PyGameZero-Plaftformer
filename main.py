...
states = ['running', 'paused', 'winning', 'title']
state = 'title'
substate = ''
state_ready = False
level = 1
    ...
    if DEBUG:
        try:
            ...
            if g.world_objects['player'].collidepoint(pos):
                g.world_objects['player'].get_hurt()
        except:
            pass
...
def update(dt):
    ...
    if state == 'title':
        menu.update()
    if state == 'running' and not state_ready:
        load_level(level)
    if state == 'running' and state_ready:
...
def draw():
    ...
    if state == 'title':
        ...
        start_button.draw()
        menu.draw()
    if state in ['running', 'paused'] and state_ready:
        screen.clear()
        draw_background()
